    ...
    <div
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition group">
                        <td class="p-6">
                            <div class="flex items-center space-x-4">
                                <div
                                    class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center font-black text-sm uppercase transition-colors">
                                    {{ substr($t->user->name, 0, 1) }}
                                </div>

                                <div>
                                    <h4 class="font-bold text-gray-800 dark:text-gray-200 tracking-tight">
                                        {{ $t->user->name }}
                                    </h4>
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <p
                                            class="text-[9px] text-green-600 dark:text-green-400 font-black uppercase tracking-widest">
                                            Aktif
                                        </p>
                                    </div>
                                </div>
                        ...
                        <td class="p-6">
                            <div class="inline-flex flex-col">
                                <span
                                    class="bg-blue-600 text-white px-3 py-1 rounded-lg font-black text-xs tracking-tighter italic">
                                    {{ $t->room->room_number }}
                                </span>

                                <span
                                    class="text-[9px] text-gray-400 dark:text-gray-500 font-black uppercase mt-1 ml-1 tracking-widest">
                                    {{ $t->room->type }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="p-6">
                            <div class="flex justify-center">
                                <form action="{{ route('tenants.destroy', $t->id) }}"
                                    method="POST"
                                    onsubmit="return confirm('Proses checkout penghuni?')">
                                    @csrf
                                    @method('DELETE')

                                    <button type="submit"
                                        class="bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 hover:bg-red-500 dark:hover:bg-red-600 hover:text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition duration-300 shadow-sm border border-red-100 dark:border-red-800/40">
                                        Checkout
                                    </button>
                                </form>
                            </div>
        ...
        @if($tenants->isEmpty())
            <div class="py-20 text-center">
                <div
                    class="inline-flex items-center justify-center w-16 h-16 bg-gray-50 dark:bg-slate-800 text-gray-300 dark:text-gray-600 rounded-full mb-4 transition-colors">
